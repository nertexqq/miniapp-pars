...
// Параллакс убран - теперь используется flip card эффект
...
// Переключение мониторинга через кнопку
if (monitoringToggleBtn && monitoringToggle) {
    // Убираем pristine класс для анимации
...
async function toggleMonitoring(enabled) {
    // Мгновенно обновляем состояние UI
    monitoringEnabled = enabled;
    monitoringToggle.checked = enabled;
    updateMonitoringStatus();
    
    // Мгновенно очищаем сетку при выключении
    if (!enabled) {
    ...
    try {
        ...
        const data = await response.json();
        // Синхронизируем с сервером только если ответ отличается
...
async function applyFiltersImmediate() {
    ...
    try {
        ...
        if (response.ok) {
            // Очищаем сетку при изменении фильтров
...
// Мгновенное применение фильтров при изменениях
document.querySelectorAll('.marketplace-checkbox').forEach(cb => {
...
if (backgroundsInput) {
    backgroundsInput.addEventListener('input', scheduleApplyFilters);
}
...
function getFilters() {
    const marketplaces = Array.from(document.querySelectorAll('.marketplace-checkbox:checked'))
        .map(cb => cb.value);
...
function updateFilters(filters) {
    ...
    if (filters.backgrounds) {
        document.getElementById('backgrounds').value = filters.backgrounds.join(', ');
    }
    
    if (filters.min_price !== undefined && filters.min_price !== null) {
        minPriceEl.value = filters.min_price;
    }
    if (filters.max_price !== undefined && filters.max_price !== null) {
        maxPriceEl.value = filters.max_price;
    }
...
// Добавление подарка в сетку
function addGift(gift) {
    const giftKey = getGiftKey(gift);
    if (seenGiftKeys.has(giftKey)) {
        return;
    }
    ...
    marketplace.onclick = async (e) => {
        e.stopPropagation();
        try {
            const response = await fetch('/api/gift_details', {
                method: 'POST',
            ...
            console.error('Error getting marketplace link:', error);
        }
        return false;
    };
    ...
    // Клик на карточку открывает выдвижную панель справа
    card.addEventListener('click', () => {
        openGiftModal(gift);
    });
    
    // Добавляем в начало сетки (новые сверху)
    ...
    // Ограничиваем количество подарков (оставляем последние 100)
    ...
    // Крутая анимация появления с bounce эффектом
...
function getGiftKey(gift) {
    ...
    const parts = [
        gift?.marketplace || 'unknown',
        gift?.name || 'unknown',
        gift?.model || 'unknown',
        gift?.gift_number || gift?.timestamp || 'unknown',
    ];
    return parts.join('|');
}
...
function ensureGiftModal() {
    ...
    giftModal.innerHTML = `
        ...
        <div class="gift-modal-content">
            <button class="gift-modal-close" type="button">
                <span class="material-symbols-outlined">close</span>
            </button>
            <div class="gift-modal-header">
                <h3 id="giftModalTitle">Подарок</h3>
            </div>
            ...
            <div class="gift-modal-sales">
                <div>
                    <a id="giftModalSalesLink" class="gift-modal-sales-link" href="#" target="_blank" rel="noreferrer">
                        ...
                        Последние продажи модели (Tonnel)
                    </a>
...
function openGiftModal(gift) {
    ...
    // Устанавливаем название маркетплейса
    ...
    fetch('/api/gift_details', {
        ...
        .then(data => {
            if (data.marketplace_link && data.marketplace_link !== '#') {
                ...
                // Ссылка на продажи ведет на тот же маркетплейс
        ...
        .catch(() => {
            modelSalesEl.innerHTML = '<li>Нет данных</li>';
        });
}
...
function renderSalesList(container, sales, giftName) {
    ...
    sales.forEach(sale => {
        ...
        if (time) {
            parts.push(formatTimestamp(time));
        }
        if (parts.length === 0) {
            item.textContent = 'Продажа';
        } else {
...
function renderTags(container, items) {
    ...
    items.forEach(item => {
        ...
        remove.addEventListener('click', () => {
            if (container === collectionsTagsEl) {
                selectedCollections = selectedCollections.filter(c => c !== item);
            } else {
                selectedModels = selectedModels.filter(m => m !== item);
            }
            renderSelectedTags();
        });
...
function ensureSelectorModal() {
    if (document.getElementById('selectorModal')) {
        return;
    }
    ...
    modal.querySelector('#selectorSearch').addEventListener('input', () => {
        ...
        renderSelectorList(filtered, type);
    });
}
...
function renderSelectorList(items, type) {
    ...
    items.forEach(item => {
        ...
        if (catalogCache) {
            if (type === 'collection') {
                // Для коллекций - берем фотку любой модели из коллекции
                ...
                // Для моделей - берем фотку конкретной модели
                imageUrl = catalogCache.model_images?.[item.name] || '';
            }
        }
        
        const imageHtml = imageUrl ? `<img src="/data/${imageUrl}" alt="${item.name}" class="selector-item-image" onerror="this.style.display='none'">` : '';
...
async function pollRecentGifts() {
    try {
        ...
        if (Array.isArray(items)) {
            items.forEach(addGift);
        }
    } catch (error) {
        // Без шума в консоли, если бэкенд недоступен
    }
}
...
function initBackgroundCanvas() {
    ...
    const draw = () => {
        ...
        blobs.forEach(blob => {
            blob.x += blob.vx;
            blob.y += blob.vy;
            if (blob.x < 0.1 || blob.x > 0.9) {
                blob.vx *= -1;
            }
            if (blob.y < 0.1 || blob.y > 0.9) {
                blob.vy *= -1;
            }
